...
| Platform proof | macOS Apple Silicon and Dockerized Linux ARM64 have clean exact-artifact proofs; native GitHub-hosted Linux x86_64 on Python 3.11/3.12/3.13, Windows, macOS Intel, and macOS Apple Silicon portable suites and package smoke installs pass | The workflow now clean-installs each exact wheel/npm pair and exercises renders, MCP, catalog, skills, schema, API import, and npm audit; retain the hosted run, archive checksums, and exact clone/preset audit with the release handoff |
